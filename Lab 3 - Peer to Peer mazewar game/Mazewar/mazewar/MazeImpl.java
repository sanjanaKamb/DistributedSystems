...
public class MazeImpl extends Maze implements Serializable, ClientListener, Runnable {
	...
	public static String dead_player = null;							
	public static Point dead_player_pt = null;
	public static volatile boolean someone_died = false;
	public static volatile boolean procesing_fire = false;
	
		/*NEW FUNCTION: Updates client (remote) target position on map*/
		public  void UpdateClientMap(Client source, Client target, GamePacket packet){
			 notifyClientFired(source); 
			 Object  o =clientMap.remove(target);
			 Point point = (Point)o;
             CellImpl cell = getCellImpl(point);
             cell.setContents(null);
             cell = getCellImpl(packet.point);
             cell.setContents(target);
             clientMap.put(target, new DirectedPoint(packet.point, packet.orientation));
             
             update();             
             
		}
		
		public void UpdateClientPosition(Point p, Client client){
			...
			Point point = (Point)o;
			 ...
			 clientMap.put(client, new DirectedPoint(p, d));
			 update();
		}
			...
			client.fire_done=true;
                ...
                if(contents != null) {
                        ...
                        if(contents instanceof Client) {
                        	assert (someone_died==false);
                        	//someone_died=true; //moved condition
                                notifyClientFired(client);
                                //kill client and record who died
				...
				client.fire_done=true;
                        ...
                        // Otherwise fail (bullets will destroy each other)
                        	someone_died=false;//enforce flag down
				//update flag
				client.fire_done=true;    
                        	return false;
                        }
        ...
        public synchronized void addRemoteClient(Client client, Point point, Direction orientation) {
            assert(client != null);
            assert(checkBounds(point));
            CellImpl cell = getCellImpl(point);
        ...
        private synchronized void killClient(Client source, Client target) {
                ...
                assert (dead_player_pt==null); 
                dead_player = target.getName();	
                dead_player_pt = point; //copy constructor
                //***********************************************************
        }
