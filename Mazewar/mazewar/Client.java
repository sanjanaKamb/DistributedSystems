...
public abstract class Client {
		...
		public static volatile boolean notify_done = false;
		public static volatile boolean fire_done = false;

		
		public void addClientThread(GameClientThread ClientThread){				//add clienthread to localclient
			this.ClientThread = ClientThread;
			
		}
		
		public GameClientThread getClientThread(){
		...
		notify_done=false;
                if(maze.clientFire(this)) {
                    notifyFire();					
					/*Spin lock to make sure fire is done before sending a packet to server about dead client*/
                    System.out.println("Spinning"+fire_done+notify_done);
					while(fire_done==false);

					System.out.println("Spinning Done"+fire_done+notify_done);
					fire_done=false;
					notify_done=false;
								return true;
                } else {
                	
					System.out.println("Spinning2"+fire_done+notify_done);
					while(!(fire_done));
					fire_done=false;
					notify_done=false;
					System.out.println("Spinning Done2"+fire_done+notify_done);
                    return false;
                }
        }
